...
/// Helper to create an error for unsupported transports.
pub fn unsupported_transport(message: impl Into<String>) -> RemoteConfigResult<FetchResponse> {
    Err(internal_error(message))
}

/// Provides the installation identifier and auth token used by the Remote Config REST API.
pub trait InstallationsProvider: Send + Sync {
    fn installation_id(&self) -> RemoteConfigResult<String>;
